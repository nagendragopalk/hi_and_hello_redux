...
import { get_All_Products, get_All_Product_details, Load_All_Products, Load_Product_detailes } from './actions';
...
export class Product_Effects {
  ...
  getProduct_details$ = createEffect(() => this.actions$.pipe(
    ...
    switchMap(action => this.productServices.get_prod_details(action.Id).pipe(
      map (data => get_All_Product_details({ get_product_details: data}))
      ))
    )
  );
}